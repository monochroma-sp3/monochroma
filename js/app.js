...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.import-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            if (importType !== 'navidrome') {
                const unameEl = document.getElementById('navidrome-username-input');
                if (unameEl) unameEl.value = '';
                const pwdEl = document.getElementById('navidrome-password-input');
                if (pwdEl) pwdEl.value = '';
                const selEl = document.getElementById('navidrome-playlist-select');
                if (selEl) selEl.innerHTML = '';
                const cntEl = document.getElementById('navidrome-playlists-container');
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#create-playlist-btn') || e.target.closest('#library-create-playlist-card')) {
            ...
            const nPlaylistsContainer = document.getElementById('navidrome-playlists-container');
            if (nUsernameInput) nUsernameInput.value = '';
            if (nPasswordInput) nPasswordInput.value = '';
            if (nPlaylistSelect) nPlaylistSelect.innerHTML = '';
            if (nPlaylistsContainer) nPlaylistsContainer.style.display = 'none';
            ...
            const nImportPanel = document.getElementById('navidrome-import-panel');
            if (nImportPanel) nImportPanel.style.display = 'none';
